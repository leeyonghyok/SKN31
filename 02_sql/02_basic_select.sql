...
-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, 
-- dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
...
-- 부서(dept_name)가 'Shipping' 이거나 2005년 이후 입사한  직원들을 조회.
...
- order by 정렬기준컬럼 정렬방식 [, ...]
    - 정렬기준컬럼 지정 단위: 컬럼이름, 컬럼의순번(select절의 선언 순서)
     `select salary, hire_date from emp` 에서 salary 컬럼을 기준으로 정렬할 경우 `order by salary` 또는
     `order by 1` 로 작성할 수있다.
